...
-- =============================================================================
-- =============================================================================
-- VERO LUXURY E-COMMERCE - SHIPPING RATES SYSTEM MIGRATION
-- =============================================================================
-- This migration provisions the official Egyptian Governorate shipping rates table.
-- Enforces absolute maximum shipping rate of 90 EGP and non-negative constraint.
-- Safe, backward-compatible, idempotent. Preserves existing orders and tables.
-- =============================================================================
...
-- Clean and idempotent RLS Policies
DO $$ BEGIN
  -- Public Read Policy: Customers can read shipping rates for checkout
  ...
  -- Full Access Policy for App Server / Service API
...
-- Auto-update updated_at timestamp trigger if function exists
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_timestamp_column') THEN
    DROP TRIGGER IF EXISTS update_shipping_rates_modtime ON public.shipping_rates;
    CREATE TRIGGER update_shipping_rates_modtime 
      BEFORE UPDATE ON public.shipping_rates 
      FOR EACH ROW EXECUTE FUNCTION update_timestamp_column();
  END IF;
END $$;

-- Ensure orders table has shipping_cost column and governorate column
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='orders' AND column_name='shipping_cost') THEN
    ALTER TABLE public.orders ADD COLUMN shipping_cost NUMERIC(12, 2) DEFAULT 0.00;
  END IF;
...
-- =============================================================================
-- SEED INITIAL 27 EGYPTIAN GOVERNORATES (IDEMPOTENT)
-- =============================================================================
